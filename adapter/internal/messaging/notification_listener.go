...
func handleApplicationEvents(data []byte, eventType string) {
	if strings.EqualFold(applicationRegistration, eventType) ||
		...
		if strings.EqualFold(removeApplicationKeyMapping, eventType) {
			...
			logger.LoggerInternalMsg.Infof("Application Key Mapping for the applicationID %s under ConsumerKey %s is removed.",
				applicationKeyMapping.ApplicationUUID, applicationKeyMappingReference)
		} else {
			eh.ApplicationKeyMappingMap[applicationKeyMappingReference] = &applicationKeyMapping
			logger.LoggerInternalMsg.Infof("Application Key Mapping for the applicationID %s is added under ConsumerKey %s.",
				applicationKeyMapping.ApplicationUUID, applicationKeyMappingReference)
		}
