...
function initApplication50PerMinPolicy() {
    ...
    forever {
        ...
        select s50PerMinreqCopy.messageID as messageID, (s50PerMinreqCopy.appTier == "50PerMin") as isEligible, s50PerMinreqCopy.appKey as throttleKey, 0 as expiryTimestamp
        ...
        select s50PerMineligibilityStream.throttleKey as throttleKey, count() as eventCount, true as stopOnQuota, s50PerMineligibilityStream.expiryTimestamp as expiryTimeStamp
        ...
        select s50PerMinintermediateStream.throttleKey, getThrottleValues50PerMin(s50PerMinintermediateStream.eventCount) as isThrottled, s50PerMinintermediateStream.stopOnQuota, s50PerMinintermediateStream.expiryTimeStamp
        group by s50PerMineligibilityStream.throttleKey
        ...
        window gateway:emitOnStateChange(s50PerMinresultStream.throttleKey, s50PerMinresultStream.isThrottled, "s50PerMinresultStream")
        select s50PerMinresultStream.throttleKey as throttleKey, s50PerMinresultStream.isThrottled, s50PerMinresultStream.stopOnQuota, s50PerMinresultStream.expiryTimeStamp
        => (gateway:GlobalThrottleStreamDTO[] counts) {
            foreach var c in counts {
